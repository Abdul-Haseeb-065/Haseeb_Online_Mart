...
from app.models.product_model import Product, ProductItem, ProductSize # type: ignore
...
def create_category_func(
    ...
    category = Category(category_name=categ_name)
    if not category:
...
def search_products_by_category_func(category_id: int, session: DB_SESSION):
    if category_id:
        ...
        ).where(Product.category_id == category_id)).all()
        if products:
            return products
        raise HTTPException(
...
def search_products_by_gender_func(gender_id: int, session: DB_SESSION):
    if gender_id:
        products = session.exec(select(Product)
        .where(Product.gender_id == gender_id)).all()
        if products:
            return products
        raise HTTPException(
...
def search_category_func(input: str, session: DB_SESSION):
    ...
    .where(Category.category_name.startswith(input))).all()
    if categories:
        return categories
...
def search_specific_size_products_func(size_id: int, session: DB_SESSION):
    results = session.exec(
        ...
        .distinct() #Ensures that the results do not contain duplicate ProductItem entries.
    ).all()
    return results
