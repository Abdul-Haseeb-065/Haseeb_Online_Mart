...
from app.settings import SECRET_KEY, ADMIN_SECRET_KEY # type: ignore
from app.main import DB_SESSION # type: ignore
from app.models.categ_auth_model import Admin # type: ignore
...
def admin_required(token: Annotated[str, Depends(oauth2_scheme)], session: DB_SESSION):
    ...
    .where(Admin.admin_kid == admin_kid)
    ).one_or_none()
    print(admin)
    print(admin_secret)
